...
class PushiConfig():
    ...
    def slurp_config_file(self, filename):
        # read pushi.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is pushi.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from pushi.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Pushi defaults...
        default_port = 9846 if (network == 'mainnet') else 19846

        # use default port for network if not specified in pushi.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from pushi.conf
