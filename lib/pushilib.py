...
def is_valid_pushi_address(address, network='mainnet'):
    ...
    pushi_version = 140 if network == 'testnet' else 55
    ...
    try:
        ...
        # rescue from exception, not a valid Pushi address
        return False

    if (address_version != pushi_version):
        return False
...
# shims 'til we can fix the pushid side
def SHIM_serialise_for_pushid(sentinel_hex):
    ...
    # superblock => "trigger" in pushid
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # pushid expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    pushid_hex = serialise(obj)
    return pushid_hex


# shims 'til we can fix the pushid side
def SHIM_deserialise_from_pushid(pushid_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(pushid_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('pushi.conf')
