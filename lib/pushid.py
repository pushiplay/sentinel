...
class PushiDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some pushid methods
    ...
    @classmethod
    def from_pushi_conf(self, pushi_dot_conf):
        from pushi_config import PushiConfig
        config_text = PushiConfig.slurp_config_file(pushi_dot_conf)
        creds = PushiConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = pushilib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = pushilib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = pushilib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_pushid_proto_version_with_sentinel_ping)
