...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, pushid):
        golist = pushid.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_pushid(pushid, item)
    ...
    @classmethod
    def import_gobject_from_pushid(self, pushid, rec):
        import decimal
        import pushilib
        import inflection
        ...
        # shim/pushid conversion
        object_hex = pushilib.SHIM_deserialise_from_pushid(object_hex)
        objects = pushilib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from pushid...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from pushid, with every run
        ...
        # get/create, then sync payment amounts, etc. from pushid - Pushid is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(pushid)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from pushid! %s" % e)
            govobj.vote_delete(pushid)
            return (govobj, None)
    ...
    def vote_delete(self, pushid):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(pushid, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, pushid, signal, outcome):
        import pushilib
        ...
        output = pushid.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = pushilib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(pushid, signal)

    def sync_network_vote(self, pushid, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = pushid.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 pushi addr, non-multisig
            if not pushilib.is_valid_pushi_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Pushi address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (pushilib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/PushiDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import pushilib
        obj_data = pushilib.SHIM_serialise_for_pushid(self.serialise())
    ...
    def prepare(self, pushid):
        try:
            object_hash = pushid.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not pushilib.is_valid_pushi_address(addr, config.network):
    ...
    def hash(self):
        import pushilib
        return pushilib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, pushid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - pushid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, pushid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - pushid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, pushid):
        now = int(time.time())
        return (self.created_at < (now - pushid.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, pushid):
        if self.is_expired(pushid):
            return False

        return True

    def is_deletable(self, pushid):
        if self.is_expired(pushid):
            return True
