...
def has_pushi_conf():
    ...
    valid_pushi_conf = False

    # ensure pushi_conf exists & readable
    #
    # if not, print a message stating that Pushi Core must be installed and
    # configured, including JSONRPC access in pushi.conf
    try:
        f = io.open(config.pushi_conf)
        valid_pushi_conf = True
    except IOError as e:
        print(e)

    return valid_pushi_conf
