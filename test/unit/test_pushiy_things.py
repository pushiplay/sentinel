...
@pytest.fixture
def valid_pushi_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Ui' if (network == 'testnet') else 'PTLvSoxnd1nT62aAgww8XS1wjjtQtywnGz'


@pytest.fixture
def invalid_pushi_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Uj' if (network == 'testnet') else 'XTLvSoxnd1nT62aAgww8XS1wjjtQtywnGz'
...
def test_valid_pushi_address():
    from pushilib import is_valid_pushi_address

    main = valid_pushi_address()
    test = valid_pushi_address('testnet')

    assert is_valid_pushi_address(main) is True
    assert is_valid_pushi_address(main, 'mainnet') is True
    assert is_valid_pushi_address(main, 'testnet') is False

    assert is_valid_pushi_address(test) is False
    assert is_valid_pushi_address(test, 'mainnet') is False
    assert is_valid_pushi_address(test, 'testnet') is True


def test_invalid_pushi_address():
    from pushilib import is_valid_pushi_address

    main = invalid_pushi_address()
    test = invalid_pushi_address('testnet')

    assert is_valid_pushi_address(main) is False
    assert is_valid_pushi_address(main, 'mainnet') is False
    assert is_valid_pushi_address(main, 'testnet') is False

    assert is_valid_pushi_address(test) is False
    assert is_valid_pushi_address(test, 'mainnet') is False
    assert is_valid_pushi_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from pushilib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % pushilib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(pushilib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(pushilib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(pushilib.blocks_to_seconds(16616)) == 2612035
